...
between the distributions of lengths of fundamental cycles in the Target
Model (Chapter 4) and the real-world target network AND between that of a
...
#%%
def compare_model_to_configuration_model(out_dir='cycle_distributions',
                                         save_models=False,
                                         max_tries=5,
    ...
    for k in range(num_instantiations):
        ...
        while model_dies_out:
            ...
            our_model_G, our_model_G_reduced, this_model_marriage_edges, this_model_marriage_distances, this_model_children_per_couple, model_dies_out, this_output_path = human_family_network(num_people,
        ...
        if use_reduced:
            our_model_G = nx.Graph(our_model_G_reduced)
        else:
...
if __name__=='__main__':
    ...
    parser.add_argument('-path_to_start_sizes', '--path_to_start_sizes', type=str, default='data_set_start_sizes.pkl', help='full path to data_set_start_sizes.pkl')
    parser.add_argument('-reduced', '--reduced', type=get_bool, default=False, help="do you want to ditch the pre-generation 0 structure?")
